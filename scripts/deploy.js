...
async function main() {
  ...
  console.log("Deployed the Contract to :-", dappazon.address);

  for (let i = 0; i < items.length; i++) {
    const transaction = await dappazon.connect(deployer).list(
      items[i].id, items[i].name, items[i].category, items[i].image, tokens(items[i].price), items[i].rating, items[i].stock)
